...
private:
    ...
    ros::Subscriber sub_save_pcs;
    ros::Publisher lc_pc_pub_final;
    DepthCamera dc;
    ...
    image_transport::Publisher rgb_pub_3d;
    image_transport::Publisher depth_pub_3d;
    RVIZPath* path_pub_3d;
    ...
    bool saveMapCallback(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
    {
      ...
      size_t n1 = kf_map_lc.size();
      PointCloudRGB::Ptr final_p(new PointCloudRGB);
      for(size_t i=0; i<n; i=i+5)
      {
        PointCloudRGB::Ptr p = generatePointCloud(kf_map_lc[i]->T_c_w.inverse(), colors[i], depths[i]);
        *final_p += *p;
      }
      sensor_msgs::PointCloud2 output_final;
      pcl::toROSMsg(*final_p,output_final);
      output_final.header.frame_id = "map";
      lc_pc_pub_final.publish(output_final);

      pcl::io::savePLYFileBinary("/home/rick/FLVIS/src/FLVIS/results/final_model.ply", *final_p);


        res.success = true;
        res.message = "Saved to point cloud.";
        return true;
    }
    open3d::geometry::Image o3d_cv(const cv::Mat& A) {
      open3d::geometry::Image B;
      int bytes_per_channel = (A.depth() / 2 + 1);
      //std::cout<<"bit of image:"<<bytes_per_channel<<" ";//refer to the fuction depth
    ...
    void save_callback(const std_msgs::Bool::ConstPtr& save)
    {
      if(saved)
        return;
      saved = true;
      ...
      size_t n = colors.size();
      std::cout<<"Save point cloud with "<<n<<" rgb-d frames."<<std::endl;

      double length = 5.0;
      int resolution = 512;
      double sdf_trunc_percentage = 0.01;

      std::vector<open3d::geometry::RGBDImage> rgbdimages;
      ...
      open3d::camera::PinholeCameraParameters  parameters;
      ...
      for(size_t i=0; i < n; i=i+1)
      {
        ...
        depthimage = o3d_cv(depths[i]);
        //std::cout<<"Convert mat to open3d"<<std::endl;
        std::shared_ptr<open3d::geometry::RGBDImage> image = open3d::geometry::RGBDImage::CreateFromColorAndDepth(rgbimage, depthimage, 1000.0, 4.5,false);
        parameters.intrinsic_.SetIntrinsics(dc.img_w, dc.img_h, dc.cam0_fx, dc.cam0_fy, dc.cam0_cx, dc.cam0_cy);
        parameters.extrinsic_ = kf_map_lc[i]->T_c_w.matrix();
        trajectory.parameters_.push_back(parameters);
        volume.Integrate(*image, parameters.intrinsic_, parameters.extrinsic_);
        r.sleep();
        rgbdimages.push_back(*image);
        trajectory_.parameters_.push_back(parameters);
      }
      std::cout<<"Volume extracted: "<<std::endl;
      auto mesh = volume.ExtractTriangleMesh();
      mesh->FilterSmoothSimple(5);
      auto point = volume.ExtractPointCloud();
      //point->RemoveStatisticalOutliers(30, 3.0);

      open3d::io::WritePointCloudOption option;

      open3d::io::WritePointCloudToPLY("/home/rick/FLVIS/src/FLVIS/results/point.ply", *point, option);

      volume.Reset();



      std::cout<<"Mesh before optimized: "<<std::endl;

      open3d::utility::SetVerbosityLevel(open3d::utility::VerbosityLevel::Debug);


//      open3d::pipelines::color_map::NonRigidOptimizerOption rigid_option;
//      rigid_option.maximum_iteration_ = 50;
//      rigid_option.maximum_allowable_depth_ = 4.5;
//      open3d::geometry::TriangleMesh optimized_mesh =
//              open3d::pipelines::color_map::RunNonRigidOptimizer(
//                      *mesh, rgbdimages, trajectory_, rigid_option);

//      open3d::io::WriteTriangleMesh("/home/rick/FLVIS/src/FLVIS/results/color_map_after_optimization.ply", optimized_mesh);
//      std::cout<<"Optimized mesh writed: "<<std::endl;

      open3d::io::WriteTriangleMesh("/home/rick/FLVIS/src/FLVIS/results/mesh.ply", *mesh);
      std::cout<<"Mesh writed: "<<std::endl;



    }
    PointCloudRGB::Ptr generatePointCloud(SE3 Twc, cv::Mat& color, cv::Mat& depth)
    {
        ...
        for(int v = 0; v<depth.rows; v+=2)
        {
            for(int u = 0; u<depth.cols; u+=2)
            {
                ...
                if(isnan(depth.at<ushort>(pt)))
                {
                    continue;
                }
                else
                {
                    float z = (depth.at<ushort>(pt))/dc.cam_scale_factor;
                    //cout<<"Depth of point: "<<z<<endl;
                    if(z>0.2&&z<=4.5)
                    {
                        Vec3 pt_w = this->dc.pixel2worldT_c_w(Vec2(u,v),Twc.inverse(),z);

                        PointRGB  p_g;
                        p_g.x = static_cast<float>(pt_w[0]);
                        p_g.y = static_cast<float>(pt_w[1]);
                        p_g.z = static_cast<float>(pt_w[2]);
                        p_g.b = color.ptr<uchar>(v)[u*3];
                        p_g.g = color.ptr<uchar>(v)[u*3+1];
                        p_g.r = color.ptr<uchar>(v)[u*3+2];

                        tmp->points.push_back(p_g);
                        width_count++;
                    }else
                    {
                        continue;
        ...
        tmp->width=width_count;
        tmp->height=1;
        tmp->is_dense = false;
        std::cout<<"generate point cloud with size="<<tmp->points.size()<<std::endl;
        return tmp;
      }
    ...
    void frame_callback(const flvis::KeyFrameConstPtr& msg)
    {
        ...
        // [1]kf.T_c_w
        // [2]kf.keyframe_id
        if(img0_unpack.channels()==3)
        {
           colors.push_back(img0_unpack.clone());
           depths.push_back(img1_unpack.clone());
           cv::cvtColor(img0_unpack,img0_unpack,CV_BGR2GRAY);
        }
        ...
        {
            ofstream sim_txt("/home/rick/FLVIS/src/FLVIS/results/sim_mat.txt");
            sim_txt.precision(5);
            for(uint64_t i = 0; i < kf_map_lc.size(); i++)
            {
    ...
    virtual void onInit()
    {
        ...
        if(vi_type_from_yaml == VI_TYPE_D435_DEPTH)
        {
            int w = getIntVariableFromYaml(configFilePath,                    "image_width");
            int h = getIntVariableFromYaml(configFilePath,                    "image_height");
            cv::Mat K0_rect = cameraMatrixFromYamlIntrinsics(configFilePath,  "cam0_intrinsics");
        ...
        lc_pc_pub_final = nh.advertise<sensor_msgs::PointCloud2>("/final_pts",1);
        sub_kf = nh.subscribe<flvis::KeyFrame>(
                    "/vo_kf",
                    10,
                    boost::bind(&LoopClosingNodeletClass::frame_callback, this, _1));
        sub_save_pcs = nh.subscribe<std_msgs::Bool>("/save_pointcloud", 10, boost::bind(&LoopClosingNodeletClass::save_callback, this, _1));
        srv_save = nh.advertiseService("save_map", &LoopClosingNodeletClass::saveMapCallback, this);
        saved = false;
        rgb_pub_3d = it.advertise("/rgb_3d",1);
        depth_pub_3d = it.advertise("/depth_3d",1);
        path_pub_3d = new RVIZPath(nh,"/path_3d","map");
    }

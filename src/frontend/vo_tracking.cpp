...
private:
  ...
  virtual void onInit()
  {
    ...
    octomap_pub     = new OctomapFeeder(nh,"/vo_octo_tracking","vo_curr_frame",1);
    ...
    if(vi_type_from_yaml==VI_TYPE_D435_DEPTH)         {cam_type=DEPTH_D435;   imu_type=NONE;}
    ...
    octomap_pub->d_camera = cam_tracker->curr_frame->d_camera;
                            ...
                            const sensor_msgs::ImageConstPtr & img1_Ptr)
  {
    ...
    if(img0_Ptr->encoding=="rgba8")
    {
      cvtColor(cvbridge_img0->image,cvbridge_img0->image,CV_RGBA2BGR);
    }
    else if((img0_Ptr->encoding=="bgra8"))
    {
      cvtColor(cvbridge_img0->image,cvbridge_img0->image,CV_RGBA2BGR);
    }
    else if(img0_Ptr->encoding=="rgb8")
    {
      cvtColor(cvbridge_img0->image,cvbridge_img0->image,CV_RGB2BGR);
    }
//    auto stop = high_resolution_clock::now();
//    auto duration = duration_cast<milliseconds>(stop - start);
//    double solving_time = (duration.count()/1000.0);
//    std::cout<<"RGB transfer time:"<<solving_time<<std::endl;


//    if( > 960)
//    {
      int w1 = cam_tracker->d_camera.img_w;
      int w2 = img0_Ptr->width;
      if(w1 < w2)
      {
        //auto start = high_resolution_clock::now();
        cv::Size dsize = cv::Size(w1,cam_tracker->d_camera.img_h);
        cv::Mat rgb_resize,depth_resize;
        cv::resize(cvbridge_img0->image, rgb_resize, dsize, 0, 0, cv::INTER_NEAREST);
        cv::resize(cvbridge_img1->image, depth_resize, dsize, 0, 0, cv::INTER_NEAREST);
        cvbridge_img0->image = rgb_resize;
        cvbridge_img1->image = depth_resize;
//        auto stop = high_resolution_clock::now();
//        auto duration = duration_cast<milliseconds>(stop - start);
//        double solving_time = (duration.count()/1000.0);
//        std::cout<<"Frame resize time:"<<solving_time<<std::endl;
      }
...
//    }
    ...
    cv::Mat color_image = cvbridge_img0->image.clone();
    this->cam_tracker->image_feed(tstamp.toSec(),
                                  cvbridge_img0->image,
                                  cvbridge_img1->image,
                                  newkf,
                                  reset_cmd);
    if(newkf&&(color_image.channels()==1)) kf_pub->pub(*cam_tracker->curr_frame,tstamp);
    if(newkf&&(color_image.channels()==3)) kf_pub->pub_color(*cam_tracker->curr_frame,color_image.clone(),tstamp);
    ...
    if(cam_type==DEPTH_D435 && frame_n%5 == 0)//
    {
//      if(color_image.channels()==1)
//        octomap_pub->pub(this->cam_tracker->curr_frame->T_c_w,cvbridge_img1->image,tstamp);
//      else
//        octomap_pub->pub_color_depth(this->cam_tracker->curr_frame->T_c_w,
//                                  color_image,cvbridge_img1->image,tstamp);
      octomap_pub->insert_frame(this->cam_tracker->curr_frame->T_c_w,
                                       color_image,cvbridge_img1->image,tstamp);

                                              ...
                                              vector<bool>& maskHas3DInf)
{
    ...
    for(size_t i=0; i<landmarks.size(); i++)
    {
        ...
        cv::Point2f pt=cv::Point2f(round(landmarks.at(i).lm_2d_undistort[0]),round(landmarks.at(i).lm_2d_undistort[1]));
        Vec3 pt3d;
        ...
        if(isnan(d_img.at<ushort>(pt)))
        {
          ...
          float d_rand;
          d_rand = 0.3 + static_cast<float>(rand())/(static_cast<float>(RAND_MAX/(0.4)));
          pt3d_c = DepthCamera::pixel2camera(Vec2(landmarks.at(i).lm_2d_undistort[0],landmarks.at(i).lm_2d_undistort[1]),
                                             this->d_camera.cam0_fx,
                                             this->d_camera.cam0_fy,
                                             this->d_camera.cam0_cx,
                                             this->d_camera.cam0_cy,
